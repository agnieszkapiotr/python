...
class World(object):
	...
	def makeTurn(self):
		...
		if self.plaga_aktywna:
			print("[PLAGA] Skrócono długość życia organizmów.")
			for org in self.organisms:
				org.liveLength = max(1, org.liveLength // 2)
			self.plaga_tury -= 1
			if self.plaga_tury <= 0:
				self.plaga_aktywna = False
				print("[PLAGA] Plaga się zakończyła.")
	...
	def wlacz_plage(self):
		self.plaga_aktywna = True
		self.plaga_tury = 2
		print("[PLAGA] Plaga została włączona, będzie działać 2 tury.")

	def dodaj_organizm(self, organizm, pozycja):
		if not self.positionOnBoard(pozycja):
			print(f"Pozycja {pozycja} poza planszą!")
			return False
		if self.getOrganismFromPosition(pozycja) is not None:
			print(f"Pozycja {pozycja} jest zajęta!")
			return False
		
		organizm.position = pozycja
		organizm.world = self
		self.organisms.append(organizm)
		print(f"Dodano {organizm.sign} na pozycji {pozycja}")
		return True
